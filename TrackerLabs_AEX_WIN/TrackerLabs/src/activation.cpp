...
bool IsActivated()
{
    std::string userDataPath = getUserData();

    FILE* fp = fopen(userDataPath.c_str(), "rb");
    if (!fp) return false;
    ...
    std::vector<unsigned char> decoded_str = atob(tokens[1]);
    
    buf = (char*)malloc(decoded_str.size()+1);
    memset(buf, 0, decoded_str.size() + 1);

    for (int i = 0; i < decoded_str.size(); i++)
    {
        buf[i] = decoded_str[i];
    }
    std::map<std::string, std::string> user_data = parse_json(buf);

    free(buf);

    std::string productKey = user_data["productKey"];
    productKey.erase(std::remove(productKey.begin(), productKey.end(), '\"'), productKey.end());
    std::string param = "{\"productKey\":\"" + productKey + "\"}";

    httplib::Client cli("http://editlabs-05071851b597.herokuapp.com");

    if (auto res = cli.Post(
        "/api/subscription/validate-product-key",
        param,
        "application/json"))
    {
        std::string result = res->body;        
        std::map<std::string, std::string> obj = parse_json(result);
        std::string expiresAt = obj["expiresAt"];
        expiresAt.erase(std::remove(expiresAt.begin(), expiresAt.end(), '\"'), expiresAt.end());
        std::time_t t = parseISO8601(expiresAt);
...
int useVerifyOnline(const char* license,bool activate)
{
    ...
    if (client.Connect("https://fxbuddy-production-eccd.up.railway.app", "ck_60bbfd050bb532fc54354a7cd5104f09a203b2d0", "cs_8ea328e5927e16aab8472579b122491cf4defcff"))
    {
        ...
        if (client.Request(url, W3Client::reqGet))
        {
            ...
            {
                std::map<std::string, std::string> obj = parse_json_license(buf,'"');
