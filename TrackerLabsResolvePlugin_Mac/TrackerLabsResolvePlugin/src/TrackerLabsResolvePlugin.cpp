...
#ifdef __APPLE__
namespace {
std::mutex g_metalFrameScratchMutex;
std::vector<float> g_metalFrameScratch;
}
#endif
...
#ifdef __APPLE__
extern void CopyBufferFromGPUToCPU(void* cmdQueue, void* mtlBuffer, float* dstCPU, size_t byteCount);
extern void CopyBufferFromCPUToGPU(void* cmdQueue, void* mtlBuffer, const float* srcCPU, size_t byteCount);
#endif

void ImageScaler::processImagesMetal()
{
#ifdef __APPLE__
    std::lock_guard<std::mutex> metalScratchLock(g_metalFrameScratchMutex);

    const OfxRectI& bounds = _srcImg->getBounds();
    const int width = bounds.x2 - bounds.x1;
    const int height = bounds.y2 - bounds.y1;

    void* inputBuf = _srcImg->getPixelData();
    void* outputBuf = _dstImg->getPixelData();

    const size_t floatCount = 4u * (size_t)width * (size_t)height;
    const size_t byteCount = floatCount * sizeof(float);
    g_metalFrameScratch.resize(floatCount);
    float* buf = g_metalFrameScratch.data();
    memset(buf, 0, byteCount);

    CopyBufferFromGPUToCPU(_pMetalCmdQ, inputBuf, buf, byteCount);
    if (!_thumb)
    {
        processing(buf, width, height);
    }

    CopyBufferFromCPUToGPU(_pMetalCmdQ, outputBuf, buf, byteCount);
...
void ImageScaler::processImagesOpenCL()
{
	...
	else
	{
		...
		memset(buf, 0, byteCount);
		clEnqueueReadBuffer(cmdQ, (cl_mem)input, CL_TRUE, 0, byteCount, buf, 0, NULL, NULL);
		
		processing(buf, width, height);

		clEnqueueWriteBuffer(cmdQ, (cl_mem)output, CL_TRUE, 0, byteCount, buf, 0, NULL, NULL);
